...
/**
 * Long-form article BODIES only. The title, date, author, category and hero
 * image live in ARTICLES (src/lib/articles-data.ts) so the visible article page
 * and its SEO metadata + JSON-LD (app/articles/[slug]/page.tsx) share one source
 * and can never drift apart.
 */
const ARTICLE_BODIES: Record<string, { sv: React.ReactNode; en: React.ReactNode }> = {
  "future-flexible-it-staffing": {
    sv: (
      <>
                ...
                <p>
                  Vi ser en tydlig förskjutning på den svenska arbetsmarknaden. Organisationer väljer i allt större utsträckning att samarbeta med bemanningsföretag för att injicera hög IT-kompetens exakt när ett projekt kräver det. Det handlar inte om nedskärningar, utan om att dimensionera rätt.
                </p>
  ...
  "skills-shortage-construction-sweden": {
    sv: (
      <>
                ...
                <blockquote>
                  &quot;En bemanningsstrategi är inte ett alternativ när rekryteringen misslyckas, utan en integrerad del av en professionell projektplanering.&quot;
                </blockquote>
                <h2>Planera personalbehovet innan projektet startar</h2>
                <p>
                  För komplexa projekt bör personalplaneringen starta minst 6 till 8 veckor innan projektet kräver bemanning. Ju mer specifik och sällsynt kompetensen är, exempelvis certifierade maskinförare, desto längre tid behövs. Kontakta oss i god tid för att diskutera ditt kommande personalbehov i Stockholm, Göteborg, Malmö eller övriga Sverige.
                </p>
              </>
    ),
    en: (
...
export default function ArticleClientPage({ slug }: { slug: string }) {
  ...
  const article = {
    title: isSv ? meta.page.titleSv : meta.page.titleEn,
    category: isSv ? meta.page.categorySv : meta.page.categoryEn,
    date: isSv ? meta.dateSv : meta.dateEn,
    author: meta.page.author,
    authorRole: isSv ? meta.page.authorRoleSv : meta.page.authorRoleEn,
    content: isSv ? body.sv : body.en,
  };
