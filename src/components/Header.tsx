...
export const Header: React.FC = () => {
  ...
  const isHomePage = normalizedPath === `/${routeLanguage}`;
  ...
  const inactiveNavLinkClass = isTransparentHomeHeader
    ? 'text-white hover:text-brand-secondary'
    : 'text-text-primary hover:text-brand-secondary';
  const mobileMenuButtonClass = isTransparentHomeHeader
    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex w-full justify-center pointer-events-none px-4">
      <header
        ...
        className={`relative grid grid-cols-2 min-[1440px]:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] ${isScrolled ? 'gap-x-2' : 'gap-x-4'} items-center pointer-events-auto w-full will-change-[max-width,border-radius,height,padding,background-color,box-shadow,transform] transition-all duration-300 ease-[cubic-bezier(0,0,0.2,1)] motion-reduce:transition-none ${
          isScrolled
            ? 'max-w-[1120px] translate-y-3 rounded-[100px] border border-border-custom/70 bg-bg-surface/95 px-6 py-1.5 shadow-lg shadow-slate-950/10 backdrop-blur-md dark:shadow-black/30'
            : isTransparentHomeHeader
              ? 'max-w-[1440px] translate-y-0 rounded-none border border-transparent bg-transparent px-6 py-[22px] shadow-none'
              : 'max-w-[1440px] translate-y-0 rounded-none border border-border-custom/70 bg-bg-surface/95 px-6 py-[22px] shadow-sm backdrop-blur-md'
        }`}
      >
        {/* Logo (Left side) */}
