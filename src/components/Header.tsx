...
export const Header: React.FC = () => {
  ...
  const localizedPath = normalizedPath.replace(/^\/(?:sv|en)/, '') || '/';
  const hasDarkTopBackground =
    localizedPath === '/' ||
    localizedPath === '/services' ||
    localizedPath.startsWith('/services/') ||
    localizedPath === '/about' ||
    localizedPath === '/team' ||
    localizedPath === '/careers' ||
    localizedPath === '/contact' ||
    localizedPath === '/articles';
  ...
  const isCompactHeader = isScrolled;
  const isTransparentHeader = !isScrolled;
  const navSpacingClass = isCompactHeader ? 'gap-2 2xl:gap-3' : 'gap-3 2xl:gap-5';
  const navLinkSizeClass = isCompactHeader ? 'px-2 py-2 text-[11px]' : 'px-2 py-2 text-sm 2xl:px-4 2xl:text-base';
  const actionGroupClass = isCompactHeader ? 'gap-1' : 'gap-2';
  const inactiveNavLinkClass = isTransparentHeader
    ? hasDarkTopBackground
      ? 'text-white hover:text-brand-secondary'
      : 'text-text-primary hover:text-brand-secondary'
    : 'text-text-primary hover:text-brand-secondary';
  const mobileMenuButtonClass = isTransparentHeader
    ? hasDarkTopBackground
      ? 'text-white hover:border-white/20 hover:bg-white/10'
      : 'text-text-primary hover:border-border-custom hover:bg-bg-surface/70'
    : 'text-text-primary hover:border-slate-300 hover:bg-slate-100 dark:hover:border-white/20 dark:hover:bg-white/10';

  return (
    <header
      ref={headerRef}
      className="fixed inset-x-0 top-0 z-50 w-full bg-transparent px-4 pointer-events-none"
    >
      <div
        className={`relative mx-auto grid grid-cols-2 min-[1440px]:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] ${isCompactHeader ? 'gap-x-2' : 'gap-x-4'} items-center pointer-events-auto will-change-[width,max-width,border-radius,height,padding,background-color,box-shadow,transform] transition-all duration-300 ease-[cubic-bezier(0,0,0.2,1)] motion-reduce:transition-none ${
          isCompactHeader
            ? 'w-full max-w-[1120px] translate-y-3 rounded-[100px] border border-border-custom/70 bg-bg-surface/95 px-6 py-1.5 shadow-lg shadow-slate-950/10 backdrop-blur-md dark:shadow-black/30'
            : 'h-[var(--navbar-height)] w-full max-w-[1440px] translate-y-0 rounded-none border border-transparent bg-transparent px-6 py-0 shadow-none'
        }`}
      >
        {/* Logo (Left side) */}
        <div className="flex min-w-0 items-center justify-start">
          <Link to={`/${routeLanguage}`} className="flex items-center group select-none shrink-0">
            <img
              ...
              className={`h-auto object-contain transition-[width] duration-300 ease-out motion-reduce:transition-none ${isCompactHeader ? 'w-[8rem]' : 'w-40'}`}
            />
          </Link>
        </div>

        {/* Centered Desktop Navigation Links */}
        <nav className={`hidden min-[1440px]:flex shrink-0 items-center justify-center whitespace-nowrap transition-transform duration-300 ease-[cubic-bezier(0,0,0.2,1)] ${isCompactHeader ? '-translate-x-20' : 'translate-x-0'} ${navSpacingClass}`}>
          {navLinks.map((link) => {
